...
def predict_result_output(event, context):
    ...
    for data in blob.download_as_string().decode('utf-8').split('\n'):
        ...
        for i in range(0,len(fuku3_h),3):
            ren_ture = '-'.join(uren_h[i:i+3])
            if ren_ture in bet_set_fuku3:
                fuku3_sum += int(uren_p[pay_count])
            pay_count += 1
    ...
    tan_sum = round((tan_sum/pred_race_num),2)
    #連対数
    ren1 = round((ren1/pred_race_num),2)
    ren2 = round((ren2/pred_race_num),2)
    ren3 = round((ren3/pred_race_num),2)
    ...
    """.format(race_date.strftime('%Y/%m/%d'),str(pred_race_num),str(tan_sum),str(ren1),str(ren2),str(ren3),str(return_tan),str(return_ren),str(return_fuku3))
